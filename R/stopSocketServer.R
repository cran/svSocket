...
stopSocketServer <- function(port = 8888) {
  ...
  for (i in 1:length(port)) {
    ...
    if (Port %in% servers) {# This port is open
      ...
      if (length(servers) == 0) {
        if (exists("SocketServers", envir = TempEnv(), inherits = FALSE))
          rm("SocketServers", envir = TempEnv())
      } else {
        assign("SocketServers", servers[servers != Port], envir = TempEnv())
      }
